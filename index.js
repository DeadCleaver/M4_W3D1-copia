...
function addBookCard({ asin, title, img, price, category }) {
    ...
    let seeAnchor = document.createElement("a");
    seeAnchor.href = `dettagli.html?q=${asin}`;
    let seeIcon = document.createElement("ion-icon");
    seeIcon.setAttribute("name", "search-outline");

    card.appendChild(cardImg);
    footContainer.appendChild(bookPrice);
    seeAnchor.appendChild(seeIcon);
    iconContainer.appendChild(seeAnchor);
...
function removeCard(event) {
    let cardToRemove = event.target.closest(".book-container");
    cardToRemove.classList.toggle("d-none");
}
